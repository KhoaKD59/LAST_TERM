...
model = tf.keras.models.load_model("breastcancer3.h5") #model m train
   ...
   if uploaded_file is not None:
    ...
    #Button: nút dự đoán sau khi up ảnh
            ...
            if Genrate_pred:    
                  ...
                  if(prediction == 0): 
                    st.write("Đây là một khối u lành tính, tuy vậy bạn hãy đi khám và đưa ra lộ trình phù hợp để làm tan biến khối u đó nhé <3")
                  if(prediction == 1): 
                    st.write("SOS!!!!!!!!!! Bạn đang có 1 khối u khá lớn, nhưng không sao, chúng ta hãy nhanh chóng đến Bác sĩ để đưa ra lời khuyên tốt nhất nhé! Mọi thứ sẽ ổn thui <3")
                  if(prediction == 2): 
                    st.write("Chúc mừng nhé, bạn đang khỏe mạnh, nhưng vẫn phải chăm lo cho sức khỏe mình thường xuyên nhé <3")
